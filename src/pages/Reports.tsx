...
export function Reports() {
  ...
  const loadReportsData = async () => {
    ...
    if ((user.role === "BRANCH_ADMIN" || user.role === "HOME_CELL_COORD") && user.branchName) {
      let pendingUnits = await ReportService.getPendingUnitReports(user.branchName);
      let allBranchUnits = await ReportService.getBranchUnitReports(user.branchName);

      if (user.role === "HOME_CELL_COORD") {
        // Under Home Cell Coordinator oversight, restrict to Cell Units only
        pendingUnits = pendingUnits.filter(u => u.unit_type === "CELL");
        allBranchUnits = allBranchUnits.filter(u => u.unit_type === "CELL");
      }
  ...
  return (
    <div className="flex flex-col gap-6 md:gap-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        <div>
          <h1 className="text-2xl md:text-3xl font-bold tracking-tight text-white mb-1">
            {user?.role === 'GLOBAL_ADMIN' ? 'Global Intelligence Review' : 
             ...
             user?.role === 'HOME_CELL_COORD' ? 'Cell Units Review Panel' :
             user?.role === 'FOUNDATION_SCHOOL' ? 'Foundation School Review' :
      ...
      {(user?.role === 'BRANCH_ADMIN' || user?.role === 'HOME_CELL_COORD') && renderBranchView()}
